...
class GenPdf:
    ...
    # данный метод генерирования шаблона пригождается для создания пропуска одному человеку и гостю/посетителю
    ...
    def create_person(self, person: Person):
        ...
        except RuntimeError:  # но всегда что-то может пойти не так, например, фото недоступно
            raise Exception(f'Фотография для {person.name + " " + person.surname} не найдена')
    ...
    def write(self, output_file: str):
        if self.already_written:  # чтобы еще раз напечатать pdf-ку надо заново инициализировать экземпляр
            raise Exception('Этот экземпляр класса уже однажды выписал пропуска (дайте ему отдохнуть, он устал). Чтобы еще раз написать pdf, надо переинициализировать его')
